...
public class DistilledCardPotion extends AbstractPotion implements CustomSavable<CardSave> {
    public static final String POTION_ID = SpireAnniversary5Mod.makeID(DistilledCardPotion.class.getSimpleName());
    ...
    public DistilledCardPotion(AbstractCard card) {
        super(potionStrings.NAME + card.name,
                POTION_ID,
    ...
    public DistilledCardPotion(CardSave save) {
        this(CardLibrary.getCopy(save.id, save.upgrades, save.misc));
    ...
    public void use(AbstractCreature target) {
        ...
        for (int i=0; i<potency; i++) {
            ...
            AbstractDungeon.actionManager.addCardQueueItem(new CardQueueItem(playing, (AbstractMonster) target, EnergyPanel.totalCount, true, true), true);
        }
    }
